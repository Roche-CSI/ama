...
from mr_core.utils import convert_to_pst
from mr_core.configs import Configs
from mr_core.models import utils
...
class BaseModel(LoggingMixin, Model):
    ...
    status = IntegerField(default=StatusEnums.PUBLIC, choices=STATUS_CHOICES)
    ...
    def create(cls, user=None, **query):
        ...
        if not query.get("created_by") or query.get("created_by") == USER_NOT_SET:
            raise ValueError("missing required parameter: created_by")

        inst = cls(**query)
        inst.is_create = True
        inst.save(user=user, force_insert=True)
    ...
    def restore(self):
        ...
        """
        super(BaseModel, self.__class__).update(
                                                # soft_delete_by=None,
                                                # soft_delete_at=None
    ...
    @classmethod
    def soft_delete(cls):
        return super(BaseModel, cls).update(
                                            # soft_delete_by=user,
        ...
        if recursive:
            ...
            for query, fk in reversed(list(dependencies)):
                ...
                if fk.null and not delete_nullable:
                    ...
                    model.delete(permanently).where(query).execute()
        return type(self).delete( permanently).where(self._pk_expr()).execute()
