...
import {FormatHighlight, MetadataHighlight} from "./MetadataHighlight";
...
const HighlightedText: React.FC<HighlightedTextProps> = ({item, keyName, highlightKeys}) => {
	const value: string = item[keyName] as string;
...
export const SearchResultCard: React.FC<SearchResultCardProps> = ({
	...
	return (
		<div className="mb-8 p-4 border border-gray-200 rounded-md hover:shadow-md transition-shadow relative">
			<h2 className="text-xl mb-2 flex text-center justify-between">
				<Link
					className="text-[#0064FF] hover:underline cursor-pointer"
					to={`/asset/${item.project_id}/${item.class_name}/${item.seq_id}/files?version=0.0.0`}
				>
					<HighlightedText item={item} keyName="title" highlightKeys={targetKeys}/>
				</Link>
			...
			<div className="text-xs text-gray-500 flex space-x-4 mb-4">
				...
				<div className="flex items-center space-x-2">
					...
					<p>Files: {item.leaf_version.num_objects}</p>
				</div>
				<div className="flex items-center space-x-2">
