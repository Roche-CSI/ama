...
const AssetClassView: React.FC<AssetClassViewProps> = ({assetClass}) => {
	...
	const onAssetClassUpdate = (formData: any) => {
		setClassData(new AssetClass(formData));
		setIsEditing(false);
	...
	const infoItems = [
		{ label: "Collection Name", value: assetClass.name, icon: <FileText className="size-4" /> },
		{ label: "Title", value: assetClass.title, icon: <FileText className="size-4" /> },
		{ label: "ID", value: assetClass.id, icon: <Info className="size-4" /> },
		{ label: "Created By", value: assetClass.created_by, icon: <CircleUser className="size-4" /> },
		{ label: "Created At", value: convertToCurrentTimeZone(assetClass.created_at, "date"), icon: <Calendar className="size-4" /> },
		{ label: "Last Updated", value: convertToCurrentTimeZone(assetClass.modified_at, "date"), icon: <Clock className="size-4" /> },
		{ label: "Type", value: assetClass.class_type || "general", icon: <Type className="size-4"/> },
	];
	...
	const title = assetClass.title.toLowerCase().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')
	return (
